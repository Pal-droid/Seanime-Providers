...
class Provider {
  ...
  async search(query: SearchOptions): Promise<SearchResult[]> {
    ...
    let url = `${this.api}/search?keyword=${encodeURIComponent(normalizedQuery)}`;
    ...
    if (data.includes("Non ci sono anime con i filtri inseriti")) {
      ...
      url = `${this.api}/search?keyword=${encodeURIComponent(normalizedQuery)}`;
  ...
  async findEpisodes(id: string): Promise<EpisodeDetails[]> {
    ...
    $('ul.episodes li a').each((index, element) => {
      let episodeId = element.attr('data-id') ?? "";
      ...
      let epNumText = element.text().trim();
  ...
  async getMp4Url(url: string): Promise<string[]> {
    ...
    // Robustly find the MP4 link using .each() and checking for file extension
    $('div.widget-body center a').each((index, element) => {
        const href = element.attr('href');
        // We prioritize links ending in .mp4 as they are the direct video files
        if (href && href.toLowerCase().includes('.mp4')) {
            videoUrl = href;
            return false; // break the loop
        }
        // Fallback: Check for "Download alternativo" or "download" attribute if strict extension check fails
        if (href && (element.text().toLowerCase().includes('alternativo') || element.attr('download') !== undefined)) {
             // Only set if we haven't found a better candidate yet
